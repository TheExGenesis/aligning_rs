...
def vpAgentProp(net, vpType, prop, fn):
    return _coconut_tail_call(newVP, net, vpType, lambda v: (fn)((_coconut.functools.partial(_coconut.getattr, net.vp.agents[v]))(prop)))

# vpSeries :: (net, string, series, fn) -> vp
@_coconut_tco
def vpSeries(net, vpType, series, fn):
    return _coconut_tail_call(newVP, net, vpType, lambda v: (fn)(series[v]))
