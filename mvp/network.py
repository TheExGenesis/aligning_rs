...
class priceNetworkParams(_coconut.collections.namedtuple("priceNetworkParams", "N, m, c, gamma, directed")):  # match data
    __slots__ = ()  # match data
    __ne__ = _coconut.object.__ne__  # match data
    def __eq__(self, other):  # match data
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  # match data
    def __hash__(self):  # match data
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  # match data
    def __new__(_cls, *_coconut_match_to_args, **_coconut_match_to_kwargs):  # match data
        ...
        if (_coconut.len(_coconut_match_to_args) <= 5) and (_coconut.sum((_coconut.len(_coconut_match_to_args) > 0, "N" in _coconut_match_to_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_to_args) > 1, "m" in _coconut_match_to_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_to_args) > 2, "c" in _coconut_match_to_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_to_args) > 3, "gamma" in _coconut_match_to_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_to_args) > 4, "directed" in _coconut_match_to_kwargs)) == 1):  # match data
            _coconut_match_temp_0 = _coconut_match_to_args[0] if _coconut.len(_coconut_match_to_args) > 0 else _coconut_match_to_kwargs.pop("N")  # match data
            _coconut_match_temp_1 = _coconut_match_to_args[1] if _coconut.len(_coconut_match_to_args) > 1 else _coconut_match_to_kwargs.pop("m")  # match data
            _coconut_match_temp_2 = _coconut_match_to_args[2] if _coconut.len(_coconut_match_to_args) > 2 else _coconut_match_to_kwargs.pop("c")  # match data
            _coconut_match_temp_3 = _coconut_match_to_args[3] if _coconut.len(_coconut_match_to_args) > 3 else _coconut_match_to_kwargs.pop("gamma")  # match data
            _coconut_match_temp_4 = _coconut_match_to_args[4] if _coconut.len(_coconut_match_to_args) > 4 else _coconut_match_to_kwargs.pop("directed")  # match data
            if (_coconut.isinstance(_coconut_match_temp_0, int)) and (_coconut.isinstance(_coconut_match_temp_1, int)) and (_coconut.isinstance(_coconut_match_temp_2, float)) and (_coconut.isinstance(_coconut_match_temp_3, float)) and (_coconut.isinstance(_coconut_match_temp_4, bool)) and (not _coconut_match_to_kwargs):  # match data
                N = _coconut_match_temp_0  # match data
                m = _coconut_match_temp_1  # match data
                c = _coconut_match_temp_2  # match data
                gamma = _coconut_match_temp_3  # match data
                directed = _coconut_match_temp_4  # match data
...
# match data
        if not _coconut_match_check:  # match data
            ...
            _coconut_match_err = _coconut_FunctionMatchError("pattern-matching failed for " "'data priceNetworkParams(N is int, m is int, c is float, gamma is float, directed is bool)  # match data'" " in " + (_coconut_match_val_repr if _coconut.len(_coconut_match_val_repr) <= 500 else _coconut_match_val_repr[:500] + "..."))  # match data
            _coconut_match_err.pattern = 'data priceNetworkParams(N is int, m is int, c is float, gamma is float, directed is bool)  # match data'  # match data
            _coconut_match_err.value = _coconut_match_to_args  # match data
            raise _coconut_match_err  # match data
# match data
        return _coconut.tuple.__new__(_cls, (N, m, c, gamma, directed))  # match data
# match data

# (N, m) -> Graph
baNetworkParams = _coconut.functools.partial(priceNetworkParams, c=0.0, gamma=1.0, directed=False)  # match data
...
def genNetwork(p):
    ...
    if (_coconut.isinstance(_coconut_match_to, priceNetworkParams)) and (_coconut.len(_coconut_match_to) == 5):
        ...
        m = _coconut_match_to[1]
        c = _coconut_match_to[2]
        gamma = _coconut_match_to[3]
        directed = _coconut_match_to[4]
        _coconut_match_check = True
    if _coconut_match_check:
        return _coconut_tail_call(gen.price_network, N=N, m=m, c=c, gamma=gamma, directed=directed)
    else:
        raise TypeError()
...
def populateSocialNetwork(graph, agentParams):
    ...
    for v in graph.get_vertices():
        agentsprop[v] = makeRandomAgent(agentParams)
