...
def optimizeIterator(fn, iter_type, iter):
    return _coconut_tail_call((np.argmax), (np.vectorize(fn))((_coconut_partial(np.fromiter, {1: iter_type}, 2))(iter)))

# optimizePartnerPick :: func -> iterator -> v
@_coconut_tco
def optimizePartnerPick(evalPartner, adversaries):
    return _coconut_tail_call(optimizeIterator, evalPartner, np.int, map(int, adversaries))
...
@_coconut_tco
def agentPickPartnerFromAgents(agents, v):
    return _coconut_tail_call(optimizePartnerPick, makeAgentIdEval(agents, v), v.out_neighbors())
