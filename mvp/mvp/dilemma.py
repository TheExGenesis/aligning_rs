...
@_coconut_tco
def makeSymmetricGame(r, s, t, p):
    return _coconut_tail_call(axl.game.Game, r=r, s=s, t=t, p=p)
@_coconut_tco
def makeSymmetricMatch(n_turns, game, strat1, strat2):
    return _coconut_tail_call(axl.Match, (strat1, strat2), turns=n_turns, game=game)
makeSymmetricEncounter = _coconut.functools.partial(makeSymmetricMatch, 1)
@_coconut_tco
def makePD():
    return _coconut_tail_call(makeSymmetricGame, r=3, p=1, s=0, t=5)
@_coconut_tco
...
def playSymmetricMatch(match):
    ...
    return match
@_coconut_tco
def getMatchAvgRewards(match):
    return _coconut_tail_call(match.final_score_per_turn)
def getMatchMoves(match):
    return match.result
# makeAsymmetricGame :: (A, B) -> nash.Game
# A,B = 2x2 reward matrices
@_coconut_tco
def makeAsymmetricGame(A, B):
    return _coconut_tail_call(nash.Game, A, B)
# getNashEq :: nash.Game
def getNashEqs(game):
    return [eq for eq in game.support_enumeration()]
@_coconut_tco
def calcDilemmaPayoffs(game, strat1, strat2):
    return _coconut_tail_call((getMatchAvgRewards), (playSymmetricMatch)(makeSymmetricEncounter(game=game, strat1=strat1, strat2=strat2)))
