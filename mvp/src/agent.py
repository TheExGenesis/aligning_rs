...
@_coconut_tco
def randArgmax(b):
    return _coconut_tail_call(np.random.choice, np.where(np.isclose(b, b.max()))[0])
def vToAgent(agents, v):
    return agents[v]
# makeEvalPartnerById :: agents -> vertex -> fn evalPartner
# fn evalPartner :: int -> float
@_coconut_tco
def makeAgentIdEval(agents, v):
    return _coconut_tail_call(_coconut_base_compose, _coconut.functools.partial(_coconut_igetitem, agents), (agents[v].evalPartner, 0))
...
class vectorAttr(_coconut.collections.namedtuple("vectorAttr", "n")):
    __slots__ = ()
    ...
    def __new__(cls, n):
        ...
        if _coconut.isinstance(_coconut_match_to, vectorAttr):
        ...
        if not _coconut_case_check_0:
            if _coconut.isinstance(_coconut_match_to, np.ndarray):
                _coconut_case_check_0 = True
            if _coconut_case_check_0:
                vec = makedata(cls, n)  # vector(v) where v is a vector should return v
        if not _coconut_case_check_0:
            if _coconut.isinstance(_coconut_match_to, int):
                _coconut_case_check_0 = True
            if _coconut_case_check_0:
                vec = makedata(cls, rand(n))
        if not _coconut_case_check_0:
            raise TypeError()
        return vec

# Constructor for null attribute
noAttr = _coconut.functools.partial(vectorAttr, 0)
...
class agent(_coconut.collections.namedtuple("agent", "pair_eval_fn dilemma_policy rec_policy attributes")):
    __slots__ = ()
    __ne__ = _coconut.object.__ne__
    def __eq__(self, other):
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)
...
@_coconut_tco
def optimizeIterator(fn, iter_type, iter):
    return _coconut_tail_call((_coconut.functools.partial(_coconut.operator.getitem, iter)), (randArgmax)((np.vectorize(fn))((_coconut_partial(np.fromiter, {1: iter_type}, 2))(iter))))

# optimizePartnerPick :: func -> iterator -> v
@_coconut_tco
def optimizeAgents(evalPartner, adversaryPool):
    return _coconut_tail_call(optimizeIterator, evalPartner, np.int, map(int, adversaryPool))

# optimizeAgentPick :: [agent] -> v -> v -> (iterator -> float)
@_coconut_tco
def optimizeAgentPick(agents, v):
    return _coconut_tail_call(_coconut.functools.partial, optimizeAgents, makeAgentIdEval(agents, v))
...
@_coconut_tco
def agentPickNeighbor(net, v):
    return _coconut_tail_call((optimizeAgentPick(net.vp.agents, v)), (list)(net.vertex(v).out_neighbors()))
