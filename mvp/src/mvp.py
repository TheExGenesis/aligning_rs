...
#Utils
def data2Dict(data):
    return dict(((name), (data2Dict(data.__getattribute__(name)))) for name in data._fields) if hasattr(data, '_fields') else data
def describeExperiment(config):
    print('Running experiment with the following config:'.format())
    pprint(data2Dict(config))
# spreadRec :: (vertex, (vertex, vertex)) -> [vertex,vertex,vertex]
@_coconut_mark_as_match
def spreadRec(*_coconut_match_to_args, **_coconut_match_to_kwargs):
    _coconut_match_check = False
    _coconut_FunctionMatchError = _coconut_get_function_match_error()
    if (_coconut.len(_coconut_match_to_args) == 1) and (_coconut.isinstance(_coconut_match_to_args[0], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to_args[0]) == 2) and (_coconut.isinstance(_coconut_match_to_args[0][1], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to_args[0][1]) == 2):
        agent = _coconut_match_to_args[0][0]
        rec = _coconut_match_to_args[0][1][0]
        neighbor = _coconut_match_to_args[0][1][1]
        if not _coconut_match_to_kwargs:
            _coconut_match_check = True
    if not _coconut_match_check:
        _coconut_match_val_repr = _coconut.repr(_coconut_match_to_args)
        _coconut_match_err = _coconut_FunctionMatchError("pattern-matching failed for " "'def spreadRec((agent,(rec,neighbor))) = [agent, rec, neighbor]'" " in " + (_coconut_match_val_repr if _coconut.len(_coconut_match_val_repr) <= 500 else _coconut_match_val_repr[:500] + "..."))
        _coconut_match_err.pattern = 'def spreadRec((agent,(rec,neighbor))) = [agent, rec, neighbor]'
        _coconut_match_err.value = _coconut_match_to_args
        raise _coconut_match_err

    return [agent, rec, neighbor]
# agentsTookRec :: [vertex] -> [vertex] -> [Boolean]
def agentsTookRec(picks, recs):
    return [picks[i] == recs[i] for i in range(len(picks))]
...
# IMPURE
def storeEpisode(ep, epName):
    print('storing episode {_coconut_format_0}'.format(_coconut_format_0=(epName)))
    epPath = storagePath + epName
    ep['net'].save(epPath + "_net.gt")
    pd.DataFrame.from_dict(ep['rec']['agents']).to_csv(epPath + "_rec.csv")
    pd.DataFrame.from_dict(ep['dilemma']).transpose().to_csv(epPath + "_dilemma.csv")

# IMPURE
def storeHistory(history, runName):
    for i, ep in enumerate(history):
        storeEpisode(ep, '{_coconut_format_0}{_coconut_format_1}'.format(_coconut_format_0=(runName), _coconut_format_1=(i)))


# runExperiment :: config -> history
def runExperiment(config, episode_n):
    describeExperiment(config)
...
# config -> graph
    g = genNetwork(networkParams)
    pprint('genNetwork: {_coconut_format_0}'.format(_coconut_format_0=(g)))
# graph -> config -> graph w/ agents
    net = populateSocialNetwork(g, agentParams)
    pprint('populateSocialNetwork: {_coconut_format_0}'.format(_coconut_format_0=(net)))
# graph -> agents
    agents = net.vp.agents
    pprint('net.vp.agents: {_coconut_format_0}'.format(_coconut_format_0=(agents)))
# fn -> mediator
    med = genMediator(mediatorParams)
    pprint('genMediator: {_coconut_format_0}'.format(_coconut_format_0=(med)))

#Rec phase functions
# medMakeRecs :: mediator -> graph -> [vertex] -> [vertex] # Mediator makes 1 recommendation for each agent
    medMakeRecs = _coconut.functools.partial(map, _coconut.functools.partial(medMakeRec, med, net))
# agentsPickNeighbor :: [vertex] -> [vertex] # Agents pick 1 agent from their neighborhood
    agentsPickNeighbor = _coconut.functools.partial(map, _coconut.functools.partial(agentPickNeighbor, net))
# (this return was made for history-keeping) agentsDecideRec :: [(vertex, (vertex, vertex))] -> [{pair: (vertex, vertex), is_rec: Boolean}] # Agent and mediator picks are compared and player pairs generated
# agentsDecideRec :: [(vertex, vertex, vertex)] -> [vertex] # Agent and mediator picks are compared and player pairs generated
    agentsDecideRec = _coconut.functools.partial(map, lambda x: agentDecideRec(agents, *x))
# medEvalPayoffs :: [vertex] -> int
    medEvalPayoffs = _coconut_base_compose(_coconut.functools.partial(filter, lambda x: x), (_coconut.functools.partial(map, medEvalPayoff), 0))
#Dilemma phase functions
# makeGames :: [(vertex,vertex)] -> game
    makeGames = _coconut.functools.partial(map, lambda x: makeEncounterFromPair(agents, *x))
# playGames :: [(vertex,vertex)] -> game
    playGames = _coconut.functools.partial(map, playEncounter)

    for i in range(episode_n):
        _agents = (list)(net.get_vertices())
        neighbors = (list)((agentsPickNeighbor)(_agents))
        recs = (list)((medMakeRecs)(_agents))
        picks = (list)((agentsDecideRec)((zip)(*[_agents, neighbors, recs])))
        takenRecs = agentsTookRec(picks, recs)
        mediatorPayoff = medEvalPayoffs(takenRecs)

        _historyRec = {'agents': dict(((agent), ({'neighbor': neighbors[agent], 'rec': recs[agent], 'pick': picks[agent]})) for agent in _agents)}

        pairs = (list)((zip)(*[_agents, picks]))
        games = makeGames(pairs)
        dilemmaResults = playGames(games)
        dilemmaMoves = map(_coconut.operator.itemgetter('moves'), dilemmaResults)
#dilemmaPayoffs = dilemmaResults |> map$(.['payoff'])

        _historyDi = dict(((pairs[i]), ({'game': games[i].game, 'moves': dilemmaMoves[i]})) for i in range(len(pairs)))
        episode = {'net': net, 'rec': _historyRec, 'dilemma': _historyDi}

        storeEpisode(episode, 'test{_coconut_format_0}'.format(_coconut_format_0=(i)))
...
# Config
    ...
    for mediatorParam in mediatorParamPool:
        ...
        config = experimentConfig(networkParams=mvpNetwork, agentParams=mvpAgent, gameParams=mvpGame, mediatorParams=mediatorParam)
        runExperiment(config, 2)
...
class experimentConfig(_coconut.collections.namedtuple("experimentConfig", "networkParams agentParams gameParams mediatorParams"), _coconut.object):
    __slots__ = ()
...
class mediatorPoolParams(_coconut.collections.namedtuple("mediatorPoolParams", "mediator_params_pool"), _coconut.object):
    __slots__ = ()
    __ne__ = _coconut.object.__ne__
    def __eq__(self, other):
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)
    ...
    def __new__(_cls, *_coconut_match_to_args, **_coconut_match_to_kwargs):
        _coconut_match_check = False
        _coconut_FunctionMatchError = _coconut_get_function_match_error()
        if (_coconut.len(_coconut_match_to_args) <= 1) and (_coconut.sum((_coconut.len(_coconut_match_to_args) > 0, "mediator_params_pool" in _coconut_match_to_kwargs)) == 1):
            _coconut_match_temp_0 = _coconut_match_to_args[0] if _coconut.len(_coconut_match_to_args) > 0 else _coconut_match_to_kwargs.pop("mediator_params_pool")
            if (_coconut.isinstance(_coconut_match_temp_0, [mediatorParams])) and (not _coconut_match_to_kwargs):
                mediator_params_pool = _coconut_match_temp_0
                _coconut_match_check = True

        if not _coconut_match_check:
            _coconut_match_val_repr = _coconut.repr(_coconut_match_to_args)
            _coconut_match_err = _coconut_FunctionMatchError("pattern-matching failed for " "'data mediatorPoolParams(mediator_params_pool is [ mediatorParams ])'" " in " + (_coconut_match_val_repr if _coconut.len(_coconut_match_val_repr) <= 500 else _coconut_match_val_repr[:500] + "..."))
            _coconut_match_err.pattern = 'data mediatorPoolParams(mediator_params_pool is [ mediatorParams ])'
            _coconut_match_err.value = _coconut_match_to_args
            raise _coconut_match_err

        return _coconut.tuple.__new__(_cls, (mediator_params_pool,))
