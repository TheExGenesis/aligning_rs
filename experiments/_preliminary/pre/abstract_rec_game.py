...
class DevMediator:
    def __init__(self, agents, strat=None):
        ...
        self.true_reward_matrix = self.makeSimilarityMatrix(
            self.agent_attribute_matrix)
        self.selfish_reward_matrix = self.invertSimMatrix(
            self.true_reward_matrix)

    def makeAttributeMatrix(self, agents):
        attr_list = [a.getAttributes() for a in agents]
        # adds dimension on the left, stacks vectors like pancakes
        attr_matrix = np.vstack(attr_list)
        return attr_matri\
    ...
    def makeSelfishRec(self, agent):
        ...
        # just an array to apply the mask and make the decision
        believed_values = np.array(self.selfish_reward_matrix[agent.id])
        ...
        # return for true mediator
        return self.agents[rec_n], self.selfish_reward_matrix[agent.id][rec_n]
...
class SelfishMediator(Mediator):
    ...
    def makeSelfishRec(self, agent):
        alt, alt_reward_a = self.estimateAgentAlt(agent)
        # threshold that selfish rec needs to be above to get accepted  # same as evaluateRec(agent, alt)
        threshold = 1 - alt_reward_a
...
class VectorAgent(Agent):
    ...
    def getBestNeighbor(self, a=None):
        if a == None:
            a = self
        # maybe it should come from observation, to avoid passing env as parameter
        self.neighbors = self.env.getNeighbors(self)
    ...
    def __init__(self, n_agents=1000, agents=None, network=None, mediator=None, agent_game=None):
        ...
        if agents == None and network == None:
            self.network = nx.barabasi_albert_graph(
                n_agents, new_edges).to_undirected()
            self.agents = [VectorAgent(
                id=i, attr_dim=attr_dim, env=self) for i in range(n_agents)]
        elif agents == None and network != None:
            self.network = network
            self.agents = [VectorAgent(id=i, attr_dim=attr_dim, env=self)
                           for i in range(network.number_of_nodes())]
        elif agents != None and network == None:
            self.network = nx.barabasi_albert_graph(
                len(self.agents), new_edges).to_undirected()
            self.agents = agents
...
class GameLoop:
    ...
    def loop(self):
        def end(x): return x > self.time_limit
