...
def makeEpisodeLog(_agents, agents, picks, neighbors, recs, games, dilemmaMoves):
    return {agent: {'policy': agents[agent].dilemma_policy, 'pick': picks[agent], 'neighbor': neighbors[agent], 'rec': recs[agent], 'game': games[agent].game, 'moves': dilemmaMoves[agent]} for agent in _agents}
...
#   dilemmaHist.to_csv(epPath + "_dilemma.csv")
    if drawNet:  # has this name bc it's the only kind but eventually I'll specify
        drawNet(epPath + "/pic.png", happenings, net)  # has this name bc it's the only kind but eventually I'll specify

@_coconut_tco
def loadEpisodeHap(hapPath):
    return _coconut_tail_call(pd.read_csv, hapPath)
...
@_coconut_tco
def medNames(storagePath, expName):
    return _coconut_tail_call((list), map(_coconut_base_compose(_coconut.operator.methodcaller("split", '/'), (_coconut.operator.itemgetter(-1), 0)), glob(f'{storagePath}/{expName}/*')))

def loadExperimentData(storagePath, expName, dataName):
    return {med: map(lambda path: loadEpisodeHap(f'{path}/{dataName}'), episodePaths(storagePath, expName, med)) for med in medNames(storagePath, expName)}


def loadExperiment(storagePath, expName):
    return loadExperimentData(storagePath, expName, 'hap.csv'), loadExperimentData(storagePath, expName, 'net.gt')
...
def consultDict(d, key):
    return d[key]

def linPayoffColorIntensity(_max, x):
    return x / _max
# def logColorIntensity(_max, x) = math.log(x, _max)

def payoff2Size(alpha, payoff):
    return 1 + alpha * payoff

def getPayoffColors(scores, net):
    payoff_color = net.new_vertex_property('vector<float>')
    intensity = _coconut.functools.partial(linPayoffColorIntensity, scores.max())
...
def drawNet(path, hap, net):
    ...
    graph_draw = d
    shapes = vpAgentProp(net, 'string', 'dilemma_policy', _coconut.functools.partial(consultDict, policyShapes))
    colors = vpAgentProp(net, 'string', 'dilemma_policy', _coconut.functools.partial(consultDict, policyColors))
    size = vpSeries(net, 'float', hap.totalScore, _coconut.functools.partial(payoff2Size, 1))
