...
res = cy_competitionFullEvo(episode_n=1000, medSet=[
                            0, 1], saveHistory=True, save=False)
# gt.stats.vertex_hist(res["graph"], 'out')
# %%
...
for w in ws:
    for i in range(n_trials):
        for ts in gameParams:
            cur_res = res[w][i][ts]
            res[w][i][ts] = cy_continueCompetitionExperiment(
                cur_res['graph'], cur_res['medStrats'], cur_res['finalStrats'], N=500, episode_n=1000, W1=w, W2=0, ts=(_T, _S), beta=0.005, k=30, medSet=[0])
